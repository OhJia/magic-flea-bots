...
Brain = (function() {
  ...
  Brain.prototype.processCallTags = function(reply, scope, async) {
    var args, callRe, data, giveup, k, lang, match, matches, obj, output, parts, promises, text;
      ...
      for (m = 0, len2 = ref1.length; m < len2; m++) {
        ...
        if (isAtomic) {
          ...
          if (match) {
            ...
            stars = [];
            if (match.length > 1) {
              for (i = n = 1, ref2 = match.length; 1 <= ref2 ? n <= ref2 : n >= ref2; i = 1 <= ref2 ? ++n : --n) {
                stars.push(match[i]);
              }
            }
          }
    ...
    if (matched) {
      ...
      for (o = 0, len3 = ref3.length; o < len3; o++) {
        ...
        for (r = 0, len5 = ref5.length; r < len5; r++) {
          ...
          for (i = s = 0, ref6 = weight; 0 <= ref6 ? s <= ref6 : s >= ref6; i = 0 <= ref6 ? ++s : --s) {
            bucket.push(rep);
          }
        }
  ...
  Brain.prototype.triggerRegexp = function(user, regexp) {
    var giveup, i, j, l, len, len1, m, match, name, opts, p, parts, pipes, ref, rep, type;
  ...
  Brain.prototype.processTags = function(user, msg, reply, st, bst, step, scope) {
    var botstars, content, data, formats, giveup, i, insert, j, l, len, m, match, n, name, output, parts, random, ref, ref1, replace, result, stars, subreply, tag, target, text, type, value;
    stars = [""];
    ...
    for (n = 0, len = formats.length; n < len; n++) {
      type = formats[n];
